...
# We used k = 3 groups, as the theory suggests. The model classified the data in 3 
# similar gropus.
...
# We cannot accept the homogeneity assumption. We try to transform the data eliminating
# outlier values.
...
# 6- Crossed Validation of the model
...
(tqda <- table(Original = iris$Species, Predicción = iris.qda$class))
diag(prop.table(tqda, 1))
sum(diag(tqda))/sum(tqda)

# If we use a QDA we improve the discrimination by a 4% (CCR = .98). The non-parametric model is better since
# we could not accept the homocedasticity and normality assumptions. 
